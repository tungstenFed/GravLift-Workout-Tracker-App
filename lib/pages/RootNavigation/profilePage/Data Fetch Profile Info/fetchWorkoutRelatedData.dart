...
Future<int> fetchNumberWorkoutSessions() async {
  List<Map<String,dynamic>> workoutSessions = await supabaseClient.from("workout_sessions").select().eq("user_id", user_id);
...
Future<List<WorkoutSession>?> fetchWorkoutSessions() async {
  ...
  try{
    List<Map<String,dynamic>> dataWSList = await supabaseClient.from("workout_sessions").select().eq("user_id", user_id);
